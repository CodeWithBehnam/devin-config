#!/usr/bin/env bash
# install.sh — Install Devin CLI global config (hooks, rules, commands, templates)
#
# Usage: ./install.sh
#
...
# 6. Merge hooks into config.json
echo ""
...
# Use python to merge the hooks key into existing config
/usr/bin/python3 -c "
import json, sys
...
# Merge hooks (replace existing hooks entirely)
